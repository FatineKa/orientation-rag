...
\begin{enumerate}
    ...
    \item \textbf{Profil étudiant} : Structure JSON complète intégrée au RAG
    \item \textbf{Génération de parcours} : Scripts opérationnels (avec et sans LLM)
\end{enumerate}

\subsection{Étape Suivante : Intégration LLM}

\textbf{Actions immédiates :}
\begin{enumerate}
    \item Obtenir une clé API (Mistral ou Groq)
    \item Installer les dépendances LLM : \texttt{pip install langchain-mistralai}
    \item Exécuter \texttt{rag\_llm.py}
\end{enumerate}

\textbf{Résultat attendu :} Parcours personnalisé avec explications, conseils et plan d'action détaillé.
